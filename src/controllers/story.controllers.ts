...
const getHyperRealFilm = async() => {
  ...
  const articles = await page.evaluate(blogItem => {
    return [...document.querySelectorAll(blogItem)].map(item => {
        ...
        const imagePath = item?.querySelector('.BlogList-item-image img')?.getAttribute('src');
        const link = 'https://hyperrealfilm.club' + item?.querySelector('.BlogList-item-title')?.getAttribute('href');
        const title = item?.querySelector('.BlogList-item-title')?.textContent
        const excerpt = item?.querySelector('.BlogList-item-excerpt p')?.textContent
        // const author = item?.querySelector('.BlogList-item-meta .Blog-meta-item--author')?.textContent
        const date = item?.querySelector('.BlogList-item-meta .Blog-meta-item--date')?.textContent

      return {imagePath, link, title, excerpt, date, id: ''}
    });
  }, blogItem);
  const finalArticles = articles.map((article) => ({...article, id: uuidv4()}))
...
const getMubi = async() => {
  ...
  const articles = await page.evaluate(blogItem => {
    return [...document.querySelectorAll(blogItem)].map(item => {
        ...
        const link = 'https://mubi.com'+item?.querySelector('.e7g0h938')?.getAttribute('href')
  ...
  const finalArticles = articles.filter(article => article.imagePath).map((article) => ({...article, id: uuidv4()}))
  await browser.close();
  return finalArticles
}
